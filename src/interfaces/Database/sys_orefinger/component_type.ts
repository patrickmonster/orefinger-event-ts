/* AUTO CREATE TABLE INTERFACE :: 1738572455766 */
/* 컴포넌트 코드 관리 */
...
export const SELECT_COMPONENT_TYPE = (where: COLUMN[], ignoreCols?: COLUMN[]) => `SELECT ${columns.filter(col => !ignoreCols?.includes(col)).join('\n	, ')}FROM sys_orefinger.component_type \nWHERE ${WHERE_COMPONENT_TYPE(where)}`
export const INSERT_COMPONENT_TYPE = (data: COLUMN[]) => ` INSERT INTO sys_orefinger.COMPONENT_TYPE SET ${WHERE_COMPONENT_TYPE(data)} `
export const UPDATE_COMPONENT_TYPE = (data: COLUMN[]) => ` UPDATE sys_orefinger.COMPONENT_TYPE SET ${WHERE_COMPONENT_TYPE(data.filter(col=> !pk.includes(col)))}`
export const DELETE_COMPONENT_TYPE = `DELETE FROM sys_orefinger.COMPONENT_TYPE`
