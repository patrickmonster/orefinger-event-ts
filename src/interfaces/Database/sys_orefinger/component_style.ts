/* AUTO CREATE TABLE INTERFACE :: 1738572455634 */
/* 컴포넌트 스타일 */
...
export const SELECT_COMPONENT_STYLE = (where: COLUMN[], ignoreCols?: COLUMN[]) => `SELECT ${columns.filter(col => !ignoreCols?.includes(col)).join('\n	, ')}FROM sys_orefinger.component_style \nWHERE ${WHERE_COMPONENT_STYLE(where)}`
export const INSERT_COMPONENT_STYLE = (data: COLUMN[]) => ` INSERT INTO sys_orefinger.COMPONENT_STYLE SET ${WHERE_COMPONENT_STYLE(data)} `
export const UPDATE_COMPONENT_STYLE = (data: COLUMN[]) => ` UPDATE sys_orefinger.COMPONENT_STYLE SET ${WHERE_COMPONENT_STYLE(data.filter(col=> !pk.includes(col)))}`
export const DELETE_COMPONENT_STYLE = `DELETE FROM sys_orefinger.COMPONENT_STYLE`
