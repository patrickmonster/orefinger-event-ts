/* AUTO CREATE TABLE INTERFACE :: 1738571724167 */
/* 컴포넌트 하위 옵션값 */
...
export const SELECT_COMPONENT_OPTION = (where: COLUMN[], ignoreCols?: COLUMN[]) => `SELECT ${columns.filter(col => !ignoreCols?.includes(col)).join('\n	, ')}FROM discord.component_option \nWHERE ${WHERE_COMPONENT_OPTION(where)}`
export const INSERT_COMPONENT_OPTION = (data: COLUMN[]) => ` INSERT INTO discord.COMPONENT_OPTION SET ${WHERE_COMPONENT_OPTION(data)} `
export const UPDATE_COMPONENT_OPTION = (data: COLUMN[]) => ` UPDATE discord.COMPONENT_OPTION SET ${WHERE_COMPONENT_OPTION(data.filter(col=> !pk.includes(col)))}`
export const DELETE_COMPONENT_OPTION = ` DELETE FROM discord.COMPONENT_OPTION`
