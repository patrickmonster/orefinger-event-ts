/* AUTO CREATE TABLE INTERFACE :: 1738571724194 */
/* 옵션 - 컴포넌트 관계 */
...
export const SELECT_COMPONENT_OPTION_CONNECTION = (where: COLUMN[], ignoreCols?: COLUMN[]) => `SELECT ${columns.filter(col => !ignoreCols?.includes(col)).join('\n	, ')}FROM discord.component_option_connection \nWHERE ${WHERE_COMPONENT_OPTION_CONNECTION(where)}`
export const INSERT_COMPONENT_OPTION_CONNECTION = (data: COLUMN[]) => ` INSERT INTO discord.COMPONENT_OPTION_CONNECTION SET ${WHERE_COMPONENT_OPTION_CONNECTION(data)} `
export const UPDATE_COMPONENT_OPTION_CONNECTION = (data: COLUMN[]) => ` UPDATE discord.COMPONENT_OPTION_CONNECTION SET ${WHERE_COMPONENT_OPTION_CONNECTION(data.filter(col=> !pk.includes(col)))}`
export const DELETE_COMPONENT_OPTION_CONNECTION = ` DELETE FROM discord.COMPONENT_OPTION_CONNECTION`
