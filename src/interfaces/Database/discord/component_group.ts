/* AUTO CREATE TABLE INTERFACE :: 1738571724088 */
/*  */
...
export const SELECT_COMPONENT_GROUP = (where: COLUMN[], ignoreCols?: COLUMN[]) => `SELECT ${columns.filter(col => !ignoreCols?.includes(col)).join('\n	, ')}FROM discord.component_group \nWHERE ${WHERE_COMPONENT_GROUP(where)}`
export const INSERT_COMPONENT_GROUP = (data: COLUMN[]) => ` INSERT INTO discord.COMPONENT_GROUP SET ${WHERE_COMPONENT_GROUP(data)} `
export const UPDATE_COMPONENT_GROUP = (data: COLUMN[]) => ` UPDATE discord.COMPONENT_GROUP SET ${WHERE_COMPONENT_GROUP(data.filter(col=> !pk.includes(col)))}`
export const DELETE_COMPONENT_GROUP = ` UPDATE discord.COMPONENT_GROUP SET use_yn = 'N'`
