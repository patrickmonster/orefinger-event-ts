/* AUTO CREATE TABLE INTERFACE :: 1738571724135 */
/* 그룹과 컴포넌트를 중재 */
...
export const SELECT_COMPONENT_LOW = (where: COLUMN[], ignoreCols?: COLUMN[]) => `SELECT ${columns.filter(col => !ignoreCols?.includes(col)).join('\n	, ')}FROM discord.component_low \nWHERE ${WHERE_COMPONENT_LOW(where)}`
export const INSERT_COMPONENT_LOW = (data: COLUMN[]) => ` INSERT INTO discord.COMPONENT_LOW SET ${WHERE_COMPONENT_LOW(data)} `
export const UPDATE_COMPONENT_LOW = (data: COLUMN[]) => ` UPDATE discord.COMPONENT_LOW SET ${WHERE_COMPONENT_LOW(data.filter(col=> !pk.includes(col)))}`
export const DELETE_COMPONENT_LOW = ` DELETE FROM discord.COMPONENT_LOW`
