/* AUTO CREATE TABLE INTERFACE :: 1738571726531 */
/* 전송용 메세지
...
export const SELECT_TEXT_MESSAGE = (where: COLUMN[], ignoreCols?: COLUMN[]) => `SELECT ${columns.filter(col => !ignoreCols?.includes(col)).join('\n	, ')}FROM discord.text_message \nWHERE ${WHERE_TEXT_MESSAGE(where)}`
export const INSERT_TEXT_MESSAGE = (data: COLUMN[]) => ` INSERT INTO discord.TEXT_MESSAGE SET ${WHERE_TEXT_MESSAGE(data)} `
export const UPDATE_TEXT_MESSAGE = (data: COLUMN[]) => ` UPDATE discord.TEXT_MESSAGE SET ${WHERE_TEXT_MESSAGE(data.filter(col=> !pk.includes(col)))}`
export const DELETE_TEXT_MESSAGE = ` UPDATE discord.TEXT_MESSAGE SET use_yn = 'N'`
