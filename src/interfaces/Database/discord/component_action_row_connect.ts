/* AUTO CREATE TABLE INTERFACE :: 1738571724020 */
/* 컴포넌트 ACTION_ROW */
...
export const SELECT_COMPONENT_ACTION_ROW_CONNECT = (where: COLUMN[], ignoreCols?: COLUMN[]) => `SELECT ${columns.filter(col => !ignoreCols?.includes(col)).join('\n	, ')}FROM discord.component_action_row_connect \nWHERE ${WHERE_COMPONENT_ACTION_ROW_CONNECT(where)}`
export const INSERT_COMPONENT_ACTION_ROW_CONNECT = (data: COLUMN[]) => ` INSERT INTO discord.COMPONENT_ACTION_ROW_CONNECT SET ${WHERE_COMPONENT_ACTION_ROW_CONNECT(data)} `
export const UPDATE_COMPONENT_ACTION_ROW_CONNECT = (data: COLUMN[]) => ` UPDATE discord.COMPONENT_ACTION_ROW_CONNECT SET ${WHERE_COMPONENT_ACTION_ROW_CONNECT(data.filter(col=> !pk.includes(col)))}`
export const DELETE_COMPONENT_ACTION_ROW_CONNECT = ` DELETE FROM discord.COMPONENT_ACTION_ROW_CONNECT`
