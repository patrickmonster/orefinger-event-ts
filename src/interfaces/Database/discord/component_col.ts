/* AUTO CREATE TABLE INTERFACE :: 1738571724060 */
/* 그룹과 컴포넌트를 중재
col */
...
export const SELECT_COMPONENT_COL = (where: COLUMN[], ignoreCols?: COLUMN[]) => `SELECT ${columns.filter(col => !ignoreCols?.includes(col)).join('\n	, ')}FROM discord.component_col \nWHERE ${WHERE_COMPONENT_COL(where)}`
export const INSERT_COMPONENT_COL = (data: COLUMN[]) => ` INSERT INTO discord.COMPONENT_COL SET ${WHERE_COMPONENT_COL(data)} `
export const UPDATE_COMPONENT_COL = (data: COLUMN[]) => ` UPDATE discord.COMPONENT_COL SET ${WHERE_COMPONENT_COL(data.filter(col=> !pk.includes(col)))}`
export const DELETE_COMPONENT_COL = ` DELETE FROM discord.COMPONENT_COL`
