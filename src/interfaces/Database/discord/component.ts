/* AUTO CREATE TABLE INTERFACE :: 1738571723952 */
/*  */
...
export const SELECT_COMPONENT = (where: COLUMN[], ignoreCols?: COLUMN[]) => `SELECT ${columns.filter(col => !ignoreCols?.includes(col)).join('\n	, ')}FROM discord.component \nWHERE ${WHERE_COMPONENT(where)}`
export const INSERT_COMPONENT = (data: COLUMN[]) => ` INSERT INTO discord.COMPONENT SET ${WHERE_COMPONENT(data)} `
export const UPDATE_COMPONENT = (data: COLUMN[]) => ` UPDATE discord.COMPONENT SET ${WHERE_COMPONENT(data.filter(col=> !pk.includes(col)))}`
export const DELETE_COMPONENT = ` DELETE FROM discord.COMPONENT`
