/* AUTO CREATE TABLE INTERFACE :: 1738571723978 */
/* 컴포넌트 ACTION_ROW */
...
export const SELECT_COMPONENT_ACTION_ROW = (where: COLUMN[], ignoreCols?: COLUMN[]) => `SELECT ${columns.filter(col => !ignoreCols?.includes(col)).join('\n	, ')}FROM discord.component_action_row \nWHERE ${WHERE_COMPONENT_ACTION_ROW(where)}`
export const INSERT_COMPONENT_ACTION_ROW = (data: COLUMN[]) => ` INSERT INTO discord.COMPONENT_ACTION_ROW SET ${WHERE_COMPONENT_ACTION_ROW(data)} `
export const UPDATE_COMPONENT_ACTION_ROW = (data: COLUMN[]) => ` UPDATE discord.COMPONENT_ACTION_ROW SET ${WHERE_COMPONENT_ACTION_ROW(data.filter(col=> !pk.includes(col)))}`
export const DELETE_COMPONENT_ACTION_ROW = ` UPDATE discord.COMPONENT_ACTION_ROW SET use_yn = 'N'`
